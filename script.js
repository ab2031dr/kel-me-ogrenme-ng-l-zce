...
// İstatistik modal aç/kapat + güncelle
...
function startQuizWithPool(src){
  if (!src || src.length < 4){
    quizQ.textContent = 'Yeterli kelime yok (en az 4 gerekir).';
    quizOpts.innerHTML = '';
...
quizFromSaved?.addEventListener('click', (e)=>{
  ...
  const saved = store.get(KEYS.SAVED, []);
  if (!saved.length){
    alert('Kaydedilmiş kelime yok. Önce kelime kaydetmelisin.');
    return;
  }
  startQuizWithPool(saved);
});
...
quizLevelBtns.forEach(btn=>{
  btn.addEventListener('click', (e)=>{
    ...
    startQuizWithPool(src);
  });
});

// Quiz yeniden başlat → seçiciye dön
